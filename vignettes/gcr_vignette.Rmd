...
LTMG -> output
list($N$: number of peaks; a $3N$ matrix: $A$, $U$, $S$; If Zcut (If 0 expression is more than 5, Zcut); Iteration number, upper limit 1000)
...
```discretization_method_1_LLR_mean```, and
```Build_R_matrix```.
...
best$result is a K*3 matrix with 1st, 2nd and 3rd columns are the A, U, S of the gene
