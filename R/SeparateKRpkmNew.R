SeparateKRpkmNewSingle <- function(x, n, q, err = 1e-10) {
  q <- max(q, min(x))
  c <- sum(x < q)
  x <- x[which(x >= q)]
  mean <- min(x) - 1
  sd <- sqrt(var(x))
  dnorms <- rep(0, length(x))
  p <- 1
  for (i in 1:n) {
    mean0 <- mean
    sd0 <- sd

    dnorms <- dnorm(x, mean0, sd0)
    pnrom.q <- pnorm(q, mean0, sd0)
    denom <- sum(dnorms) + pnrom.q * c
    im <- InverseMillsRatio(q, mean0, sd0)
    mean <- sum(x * dnorms + (mean0 - sd0 * im) * pnrom.q * c) / denom
    sd <- sqrt((sum((x - mean0)^2 * dnorms) + sd0 ^ 2 * (1 - (q - mean0) / sd0 * im) *
                  pnrom.q * c) / denom)
    if (is.na(sd)) {
      p <- NaN
      mean <- NaN
    ...
    if ((mean(abs(mean - mean0)) <= err) && (mean(abs(sd - sd0)) <= err)) {
      break
    }
  }
...
SeparateKRpkmNew <- function(x, n, q, k, err = 1e-10) {
  if (k == 1) return(SeparateKRpkmNewSingle(x, n, q, err = 1e-10))
  q <- max(q, min(x))
  c <- sum(x < q)
  x <- x[which(x >= q)]
  if (length(x) <= k) {
    warning("length(x) > k is not TRUE")
    return(cbind(0, 0, 0))
  }
  mean <- c()
  for (i in 1:k) {
